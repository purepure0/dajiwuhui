...
+ (NSURLSessionTask *)POST:(NSString *)URL
    ...
    /** output generate networking log*/
    [self outPutRequestURL:URL paramters:parameters];
//    PPLog(@"%@%@",URL,parameters);
...
#pragma mark - Generate Networking Log -
+ (void)outPutRequestURL:(NSString *)URL paramters:(id)paramters {
    
    NSString *url = NSStringFormat(@"%@%@",URL,@"?");
    
    NSMutableString *jointParamter = [[NSMutableString alloc] init];
    
    [paramters enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        
        [jointParamter appendString:NSStringFormat(@"&%@=%@", key, obj)];
    }];
    
    PPLog(@"%@",NSStringFormat(@"%@%@",url,[jointParamter hasPrefix:@"&"] ? [jointParamter substringFromIndex:1] : jointParamter));
}
