...
@interface SquareDanceViewController ()<UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@implementation SquareDanceViewController
...
- (void)initDataSource {
    ...
    [PPNetworkHelper GET:NSStringFormat(@"%@%@", kApiPrefix, kAdult) parameters:nil success:^(id responseObject) {
        PPLog(@"%@", responseObject);
        NSString *code = [NSString stringWithFormat:@"%@", responseObject[@"code"]];
        if ([code isEqualToString:@"0"]) {
            NSDictionary *videoIndex = responseObject[@"data"][@"Video_adult"];
